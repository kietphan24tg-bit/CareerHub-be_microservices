#!/usr/bin/env node
import {
  getApplicationCacheDlqTopology,
  peekApplicationCacheDlq,
  replayApplicationCacheDlq
} from './replay-application-cache-dlq';
...
function printHelp(): void {
  console.log(`application-cache-dlq — inspect and replay application dashboard cache invalidation DLQ messages

Usage:
  application-cache-dlq list [--count N]
  application-cache-dlq peek [--count N]
  application-cache-dlq replay [--count N] [--message-id <id>] [--dry-run]
...
Environment:
  Uses the same BROKER_* variables as application-service (BROKER_URL, BROKER_EXCHANGE_PREFIX, etc.)

Examples:
  application-cache-dlq list --count 5
  application-cache-dlq replay --dry-run
  application-cache-dlq replay --message-id outbox-record-123
  application-cache-dlq replay --count 10
`);
}
...
function printReplayResults(results: Awaited<ReturnType<typeof replayApplicationCacheDlq>>): void {
...
async function main(): Promise<void> {
  ...
  const topology = getApplicationCacheDlqTopology();
  console.log(
  ...
  if (options.command === 'list' || options.command === 'peek') {
    const peekResult = await peekApplicationCacheDlq({ count: options.count });
  ...
  const replayResults = await replayApplicationCacheDlq({
    count: options.count,
...
main().catch((error: unknown) => {
  ...
  console.error(`application-cache-dlq failed: ${message}`);
  process.exitCode = 1;
});
