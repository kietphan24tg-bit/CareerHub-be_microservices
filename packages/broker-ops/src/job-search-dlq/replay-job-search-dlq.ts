...
export const JOB_SEARCH_EVENTS_EXCHANGE = 'events';
export const JOB_SEARCH_INDEX_QUEUE = 'job.search-index';

export type JobSearchDlqReplayOutcome =
  | 'dry_run'
  | 'not_found'
  | 'publish_failed'
  | 'replayed';

export type JobSearchDlqReplayResult = {
  outcome: JobSearchDlqReplayOutcome;
  parsed?: ParsedDlqMessage;
  error?: string;
};

export type JobSearchDlqPeekResult = {
  messages: ParsedDlqMessage[];
  queueDepth: number;
};

export type JobSearchDlqReplayOptions = {
  count?: number;
  dryRun?: boolean;
  maxScan?: number;
  messageId?: string;
...
function resolveTopology(runtimeConfig: RuntimeConfig) {
  return getRabbitMqParkingDeadLetterTopology(
    runtimeConfig,
    JOB_SEARCH_EVENTS_EXCHANGE,
    JOB_SEARCH_INDEX_QUEUE
  );
}

export async function peekJobSearchDlq(
  options: JobSearchDlqReplayOptions,
  dependencies?: Partial<ReplayDependencies>
): Promise<JobSearchDlqPeekResult> {
...
): Promise<JobSearchDlqReplayResult> {
...
export async function replayJobSearchDlq(
  options: JobSearchDlqReplayOptions,
  dependencies?: Partial<ReplayDependencies>
): Promise<JobSearchDlqReplayResult[]> {
  const runtimeConfig = options.runtimeConfig ?? loadRuntimeConfig();
  const topology = resolveTopology(runtimeConfig);
  const results: JobSearchDlqReplayResult[] = [];
  ...
  try {
    return await replayJobSearchDlq(options, liveDependencies);
  } finally {
    await channel.close().catch(() => undefined);
    await connection.close().catch(() => undefined);
  }
...
export function getJobSearchDlqTopology(runtimeConfig?: RuntimeConfig) {
