...
export const JOB_SLUG_CACHE_EVENTS_EXCHANGE = 'events';
export const JOB_SLUG_CACHE_INVALIDATION_QUEUE = 'job.slug-cache-invalidation';

export type JobSlugCacheDlqReplayOutcome =
  | 'dry_run'
  | 'not_found'
  | 'publish_failed'
  | 'replayed';

export type JobSlugCacheDlqReplayResult = {
  outcome: JobSlugCacheDlqReplayOutcome;
  parsed?: ParsedDlqMessage;
  error?: string;
};

export type JobSlugCacheDlqPeekResult = {
  messages: ParsedDlqMessage[];
  queueDepth: number;
};

export type JobSlugCacheDlqReplayOptions = {
  count?: number;
  dryRun?: boolean;
  maxScan?: number;
  messageId?: string;
...
function resolveTopology(runtimeConfig: RuntimeConfig) {
  return getRabbitMqParkingDeadLetterTopology(
    runtimeConfig,
    JOB_SLUG_CACHE_EVENTS_EXCHANGE,
    JOB_SLUG_CACHE_INVALIDATION_QUEUE
  );
}

export async function peekJobSlugCacheDlq(
  options: JobSlugCacheDlqReplayOptions,
  dependencies?: Partial<ReplayDependencies>
): Promise<JobSlugCacheDlqPeekResult> {
...
): Promise<JobSlugCacheDlqReplayResult> {
...
export async function replayJobSlugCacheDlq(
  options: JobSlugCacheDlqReplayOptions,
  dependencies?: Partial<ReplayDependencies>
): Promise<JobSlugCacheDlqReplayResult[]> {
  const runtimeConfig = options.runtimeConfig ?? loadRuntimeConfig();
  const topology = resolveTopology(runtimeConfig);
  const results: JobSlugCacheDlqReplayResult[] = [];
  ...
  try {
    return await replayJobSlugCacheDlq(options, liveDependencies);
  } finally {
    await channel.close().catch(() => undefined);
    await connection.close().catch(() => undefined);
  }
...
export function getJobSlugCacheDlqTopology(runtimeConfig?: RuntimeConfig) {
