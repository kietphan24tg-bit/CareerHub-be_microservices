#!/usr/bin/env node
import {
  getJobSlugCacheDlqTopology,
  peekJobSlugCacheDlq,
  replayJobSlugCacheDlq
} from './replay-job-slug-cache-dlq';
...
function printHelp(): void {
  console.log(`job-slug-cache-dlq — inspect and replay job slug cache invalidation DLQ messages

Usage:
  job-slug-cache-dlq list [--count N]
  job-slug-cache-dlq peek [--count N]
  job-slug-cache-dlq replay [--count N] [--message-id <id>] [--dry-run]
...
Examples:
  job-slug-cache-dlq list --count 5
  job-slug-cache-dlq replay --dry-run
  job-slug-cache-dlq replay --message-id outbox-record-123
  job-slug-cache-dlq replay --count 10
`);
}
...
function printReplayResults(results: Awaited<ReturnType<typeof replayJobSlugCacheDlq>>): void {
...
async function main(): Promise<void> {
  ...
  const topology = getJobSlugCacheDlqTopology();
  console.log(
  ...
  if (options.command === 'list' || options.command === 'peek') {
    const peekResult = await peekJobSlugCacheDlq({ count: options.count });
  ...
  const replayResults = await replayJobSlugCacheDlq({
    count: options.count,
...
main().catch((error: unknown) => {
  ...
  console.error(`job-slug-cache-dlq failed: ${message}`);
  process.exitCode = 1;
});
