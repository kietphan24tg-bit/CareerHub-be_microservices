...
export class CandidateOutboxPublisher implements OnModuleDestroy {
  private readonly logger = new Logger(CandidateOutboxPublisher.name);
  private readonly candidateRuntimeConfig: CandidateRuntimeConfig;
  ...
  constructor(
    ...
    this.candidateRuntimeConfig = getCandidateRuntimeConfig(configService);
    this.runtimeConfig = getRuntimeConfig(configService);
  }

  isEnabled(): boolean {
    return (
      this.candidateRuntimeConfig.outboxPublishEnabled &&
