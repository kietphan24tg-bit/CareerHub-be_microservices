...
export class OfferOperations {
  ...
  async sendOffer(employerIdentityId: string, offerId: string): Promise<ApplicationOfferRecord> {
    ...
    if (application.status !== 'offer') {
      await this.applicationRepository.updateStatus(application.id, 'offer');
      await this.applicationRepository.createHistory({
        actorIdentityId: employerIdentityId.trim(),
        actorType: 'employer',
        applicationId: application.id,
    ...
    const historyStatus = lifecycleHistoryStatus(
      application.status === 'offer' ? 'offer' : 'offer'
    );
    await this.applicationRepository.createHistory({
      actorIdentityId: employerIdentityId.trim(),
      actorType: 'employer',
  ...
  async updateOffer(
    ...
    const updated = await this.recruitmentRepository.updateOffer(
      offer.id,
      {
        bonusDetails:
        ...
        currency: input.currency === undefined ? undefined : normalizeNullableString(input.currency),
        departmentTeam:
        ...
        expiresAt:
          input.offerExpiresAt === undefined
            ...
            : input.offerExpiresAt
              ...
              : null,
        location: input.location === undefined ? undefined : normalizeNullableString(input.location),
  ...
  async softDeleteOffer(
    ...
    const now = new Date();
    const deleted = await this.recruitmentRepository.softDeleteOffer(offer.id, now);
    if (!deleted) {
      throw new OfferNotFoundError(offerId);
    }
  ...
  async acceptOffer(
    candidateIdentityId: string,
    offerId: string,
    input: CandidateOfferDecisionInput
  ): Promise<ApplicationOfferRecord> {
    await this.recruitmentRepository.expireOfferIfDue(offerId.trim(), new Date());
    ...
    await this.applicationRepository.updateStatus(application.id, 'hired');
    await this.applicationRepository.createHistory({
      actorIdentityId: candidateIdentityId.trim(),
      actorType: 'candidate',
      applicationId: application.id,
  ...
  async declineOffer(
    ...
    await this.applicationRepository.updateStatus(application.id, 'rejected');
    await this.applicationRepository.createHistory({
      actorIdentityId: candidateIdentityId.trim(),
      actorType: 'candidate',
      applicationId: application.id,
